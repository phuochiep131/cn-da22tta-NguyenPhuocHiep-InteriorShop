...
export default function ProductDetail() {
  ...
  const handleAddToCart = (prod) => {
    ...
    messageApi.success(
      `Đã thêm ${quantity} ${selected.productName} vào giỏ hàng!`
    );
  };
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-10">
      ...
      <div className="mt-10 space-y-2">
        ...
        <p>
          <b>Kích thước:</b> {product.size || "Không có thông tin"}
        </p>
        <p>
          <b>Màu sắc:</b> {product.color || "Không có thông tin"}
        </p>
        <p>
          <b>Chất liệu:</b> {product.material || "Không có thông tin"}
        </p>
        <p>
          <b>Bảo hành:</b> {product.warranty || "Không có thông tin"}
        </p>
        <p>
          <b>Xuất xứ:</b> {product.origin || "Không có thông tin"}
        </p>
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
          {relatedProducts.map((prod) => {
            ...
            return (
              <div
                key={prod.productId}
                className="group bg-white rounded-xl shadow hover:shadow-lg transition-all duration-300 flex flex-col"
              >
                <img
                  src={prod.imageUrl}
